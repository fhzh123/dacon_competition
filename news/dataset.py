...
class CustomDataset(Dataset):
    def __init__(self, total_src_list, title_src_list, content_src_list, 
        ...
        data = list()
        for total_src, title_src, content_src, date, order, trg_or_id in zip(
            total_src_list, title_src_list, content_src_list, date_list, ord_list, trg_or_id_list
        ):
            if min_len <= len(total_src) <= max_len:
                data.append((total_src, title_src, content_src, date, order, trg_or_id))
        
        self.data = data
        self.num_data = len(self.data)
        
    def __getitem__(self, index):
        total_src, title_src, content_src, date, order, trg_or_id = self.data[index]
        return total_src, title_src, content_src, date, order, trg_or_id
